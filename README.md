...
Automated builds of the image are available on [Dockerhub](https://hub.docker.com/r/nogsantos/postgresql) and is the recommended method of installation.

```bash
docker pull nogsantos/postgresql:9.5-4
```

Alternatively you can build the image yourself.

```bash
docker build -t nogsantos/postgresql github.com/nogsantos/docker-postgresql
```
...
```bash
docker run --name postgresql -itd --restart always \
  nogsantos/postgresql:9.5-4 -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  nogsantos/postgresql:9.5-4 -c logging_collector=on
```
